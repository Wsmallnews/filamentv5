...
|--------------------------------------------------------------------------
|
| 以下语言行是 Laravel 标准验证规则的默认错误消息，译文取自
| laravel-lang/lang 项目 zh_CN 标准数据。未收录的 key 自动回退英文。
|
*/
...
return [
    ...
    |--------------------------------------------------------------------------
    |
    | 在此可按 "属性名.规则名" 维度覆盖任意字段的消息，
    | 例：'title' => ['required' => '标题不能为空']。
    |
    */
    ...
    |--------------------------------------------------------------------------
    |
    | 此处的语言行用于将 :attribute 占位符替换为更友好的字段名，
    | 例：'email' => '邮箱地址'。
    |
    */
