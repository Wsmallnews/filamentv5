...
return new class extends Migration
{
    ...
    public function up()
    {
        Schema::create(config('sn-order.order_table_name', 'sn_orders'), function (Blueprint $table) {
            $table->comment('订单');
            $table->engine = 'InnoDB';
            $table->id();
            ...
            $table->morphs('buyer');
            $table->string('currency', 3)->default('CNY')->comment('交易币种(ISO 4217)，创建时快照');
            ...
            $table->unsignedBigInteger('relate_original_amount')->default(0)->comment('原项目总金额');
            $table->unsignedBigInteger('relate_amount')->default(0)->comment('项目总金额');
            $table->unsignedBigInteger('order_amount')->default(0)->comment('订单总金额');
            ...
            $table->unsignedBigInteger('pay_fee')->default(0)->comment('支付总金额');
            $table->unsignedBigInteger('original_pay_fee')->default(0)->comment('原始支付总金额');
            $table->unsignedBigInteger('remain_pay_fee')->default(0)->comment('剩余支付金额');
    ...
    {
        Schema::dropIfExists(config('sn-order.order_table_name', 'sn_orders'));
    }
};
